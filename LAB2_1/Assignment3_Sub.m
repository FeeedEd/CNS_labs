...
for iteration=1:epochs
    ...
    for pattern=1:dataLength
        ...
        v = w' * randomInputU;
        % update the weights
        w = w + eta*(v*randomInputU - (v*(n.*randomInputU)*n)/N);
...
savefig('./images/sub_scattereig.fig');

%%
plot(wUpdated(1,:));
xlabel('Epochs')
ylabel('w1');
savefig('./images/sub_w1.fig');

%%
plot(wUpdated(2,:));
xlabel('Epochs')
ylabel('w2');
savefig('./images/sub_w2.fig');

%%
plot(wNormalized);
xlabel('Epochs')
ylabel('wNormalized');
savefig('./images/sub_wnorm.fig');

%%
save('sub_summary.mat', 'wUpdated');